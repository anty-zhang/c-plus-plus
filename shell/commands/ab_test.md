...
This is ApacheBench, Version 2.3
    ...
    #最重要的指标之一，每秒能处理的的请求数=1000ms／2.961，相当于LR中的每秒事务数，后面括号中的mean表示这是一个平均值
    Time per request: 2961.449 [ms] (mean)
    #最重要的指标之二，每批请求处理完成的平均时间，相当于LR中的平均事务响应时间，后面括号中的mean表示这是一个平均值
    Time per request: 2.961 [ms] (mean, across all concurrent requests)
    #每个请求处理完成的平均时间 =每批完成时间2961.449除以并发数 100 每个连接请求实际运行时间的平均值
